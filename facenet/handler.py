...
from commons.deepface_preprocess import normalize_input, resize_image
...
class FaceNetHandler(BaseHandler):
    ...
    def inference(self, data):
        ...
        output_tensor = []
        for image in data:
            img = resize_image(image, (160, 160))
            img = normalize_input(img, normalization="Facenet")
            batch = np.expand_dims(img, axis=0)
            emb = self.model.foward(batch)
            output_tensor.append(emb)
