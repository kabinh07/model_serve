...
class EasyOCRHandler(BaseHandler):
    ...
    def initialize(self, ctx):
        ...
        print(f"Loading English EasyOCR model from {model_dir}...")
        self.english_reader = easyocr.Reader(
            ['en'],
            gpu=gpu_available,
            model_storage_directory=model_dir,
            download_enabled=False
        )
        print(f"✅ English EasyOCR reader initialized!")
        
        self.initialized = True
        print(f"✅ EasyOCR Model initialized successfully!")
        print(f"   Loaded Languages: ['bn', 'en']")
    ...
    def preprocess(self, data):
        ...
        lang = self._extract_language(data)
        
        print(f"🔍 PREPROCESS - Received {len(bboxes) if bboxes else 0} bboxes, Language: {lang}")
        ...
        # For Bengali, convert based on model config; for English, use RGB
        if lang == 'en':
            img = img.convert("RGB")
        elif self.opt.get('rgb', False):
            img = img.convert("RGB")
        else:
            img = img.convert("L")
    ...
    def inference(self, model_input):
        ...
        print(f"🔍 INFERENCE - Processing {len(bboxes)} bounding boxes for language: {lang}")
        
        # Route to appropriate inference method based on language
        if lang == 'en':
            return self._inference_english(img, bboxes)
        else:
            return self._inference_bengali(img, bboxes)
    
    def _inference_english(self, img, bboxes):
        """
        Run English OCR using EasyOCR reader.
        Uses temporary file strategy for maximum accuracy.
        """
        output = []
        ...
        for bbox_idx, bbox in enumerate(bboxes):
            temp_file_path = None
            try:
                # Parse coordinates and ensure they are within bounds
                ...
                print(f"🔍 EN Bbox {bbox_idx}: Original bbox={bbox}, Image size={img.size}")
                print(f"🔍 EN Bbox {bbox_idx}: Parsed coords: x1={x1}, y1={y1}, x2={x2}, y2={y2}")
                
                # Check for invalid crops
                if x2 <= x1 or y2 <= y1:
                    print(f"⚠️ EN Bbox {bbox_idx} invalid dimensions, skipping.")
                ...
                # Save to temporary file (.png for lossless)
                with tempfile.NamedTemporaryFile(delete=False, suffix='.png') as temp_img:
                    crop_img.save(temp_img.name)
                    temp_file_path = temp_img.name
                
                # Use recognize() since we already have cropped text regions
                crop_w, crop_h = crop_img.size
                horizontal_list = [[0, crop_w, 0, crop_h]]  # x_min, x_max, y_min, y_max
                print(f"🔍 EN Bbox {bbox_idx}: Calling recognize()...")
                ocr_results = self.english_reader.recognize(temp_file_path, horizontal_list=horizontal_list, free_list=[])
                ...
                if ocr_results:
                    ...
                    print(f"✅ EN Bbox {bbox_idx}: '{log_text}' (Conf: {avg_conf:.4f})")
                    ...
                    print(f"⚠️ EN Bbox {bbox_idx}: No text detected")
                    output.append({"text": "", "confidence": 0.0})
                    
            except Exception as e:
                print(f"❌ EN INFERENCE - Error processing bbox {bbox_idx}: {e}")
                output.append({"text": "", "confidence": 0.0})
    ...
    def _inference_bengali(self, img, bboxes):
        """
        Run Bengali OCR using the custom trained model.
        """
        # Ensure correct image mode for Bengali model
        if img.mode == 'RGB' and not self.opt.get('rgb', False):
            img = img.convert('L')
        
        print(f"🔍 BN INFERENCE - Processing {len(bboxes)} bounding boxes")
        ...
        for idx, bbox in enumerate(bboxes):
            ...
            print(f"🔍 BN INFERENCE - Bbox {idx}: {[x1,y1,x2,y2]}, Crop shape: {cropped.shape}")
        ...
        print(f"🔍 BN INFERENCE - Batch tensor shape: {batch_tensor.shape}")
        ...
        with torch.no_grad():
            ...
            for idx, (pred, pred_max_prob) in enumerate(zip(preds_str, preds_max_prob)):
                ...
                print(f"🔍 BN INFERENCE - Result {idx}: text='{pred}', conf={confidence_score:.4f}")
        ...
        print(f"🔍 BN INFERENCE - Returning {len(output)} results")
        return output
    ...
    def _extract_language(self, row):
        ...
        # Default to Bengali if not specified or invalid
